...
import com.sunsshen.bigdata.zookeeper.test.ConcurrentTest.ConcurrentTask;
	...
	public static void main(String[] args) {
		Runnable task1 = new Runnable() {
			public void run() {
				...
				try {
					lock = new DistributedLock(ZKConnection.hosts, "test1");
		...
		for (int i = 0; i < tasks.length; i++) {
			ConcurrentTask task3 = new ConcurrentTask() {
				public void run() {
					DistributedLock lock = null;
					try {
						lock = new DistributedLock(ZKConnection.hosts, "test2");
						lock.lock();
						System.out.println("Thread "
								+ Thread.currentThread().getId() + " running");
					} catch (Exception e) {
